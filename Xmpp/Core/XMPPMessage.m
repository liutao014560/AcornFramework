...
//Done(Shaozl)://is group chat and svg chat type
...
//ADD BY (Shaozl)://
- (BOOL)isGroupChatMessageWithSvg
{
    if ([self isGroupChatMessageWithBody])
    {
        NSDictionary *pDictionary = [self.body objectFromJSONString];
        
        if (pDictionary != nil)
        {
            if ([[pDictionary valueForKey:@"type"] isEqualToString:@"svg"])
            {
                return YES;
            }
        }
...
-(BOOL)isGroupChatMessageWithRTC
{
    if ([self isChatMessageWithBody])
    {
        ...
        if (pDictionary != nil)
        {
            if ([[pDictionary valueForKey:@"type"] isEqualToString:@"rtc"])
            {
                return YES;
            }
        }
...
-(BOOL)isGroupChatMessageWithFile
{
    if ([self isGroupChatMessageWithBody])
    {
        NSDictionary *pDictionary = [self.body objectFromJSONString];
        ...
        if (pDictionary != nil)
        {
            if ([[pDictionary valueForKey:@"type"] isEqualToString:@"file"])
            {
                return YES;
            }
        }
...
-(BOOL)isBroadcastMessageWithQueue
{
    if (![self isChatMessage] && [self isMessageWithBody])
    {
        NSDictionary *pDictionary = [self.body objectFromJSONString];
        
        if (pDictionary != nil)
        {
            if ([[pDictionary valueForKey:@"type"] isEqualToString:@"queue"])
            {
                return YES;
            }
        }
...
-(BOOL)isBroadcastMessageWithInterrupted
{
    if (![self isChatMessage] && [self isMessageWithBody])
    {
        NSDictionary *pDictionary = [self.body objectFromJSONString];
        ...
        if (pDictionary != nil)
        {
            if ([[pDictionary valueForKey:@"type"] isEqualToString:@"interrupted"])
            {
                return YES;
            }
        }
